...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['tambah'])) {
    ...
    // Cek apakah gambar diupload dan valid
    if (isset($_FILES["image"])) {
        ...
        // Cek apakah ekstensi file yang diizinkan
        if (!in_array($imageFileType, $allowed_extensions)) {
            echo "Maaf, hanya file gambar JPG, JPEG, PNG, atau GIF yang diperbolehkan.";
            exit;
        }

        // Coba untuk memindahkan file gambar ke folder tujuan
        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
            echo "File " . htmlspecialchars(basename($_FILES["image"]["name"])) . " berhasil diunggah.";
        } else {
    ...
    <div class="container">
        ...
        <form method="post" action="" enctype="multipart/form-data" class="mt-4">
            ...
            <div class="form-group">
                ...
                <input type="number" name="physical_condition" class="form-control" required>
            </div>
            <div class="form-group">
                <label>Kondisi Mesin:</label>
                <input type="number" name="engine_condition" class="form-control" required>
            </div>
            <div class="form-group">
                <label>Harga:</label>
